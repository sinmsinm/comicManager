	...
	$.fn.fileUploader = function(config){
		...
		wrapper = ' \
			<div id="'+ pxWidget +'" class="px-widget ui-widget"> \
				<div id="'+pxWidget+'_droparea" class="droparea" ></div>  \
		...
		pxButton = '#' + pxButton,
		pxWidget = '#' + pxWidget,
		pxWidgetDroparea =  pxWidget + '_droparea',
		buttonClearId = null,
		...
		*/
		...
		$(pxWidgetDroparea).bind( 'dragenter dragover', false)
		...
		*/
		function afterEachUpload(formId, data, status, errorThrown) {
			...
			config.afterEachUpload(data, status,$uploadData,formId);
